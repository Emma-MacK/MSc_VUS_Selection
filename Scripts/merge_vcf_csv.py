...
exomiser_csv = exomiser_csv.dropna(subset=['CADD_PHRED'])
exomiser_csv = exomiser_csv[exomiser_csv['CADD_PHRED'] >= 22 ]


# remove high sift
exomiser_csv['sift'] = pd.to_numeric(exomiser_csv['sift'], errors='coerce')
exomiser_csv  = exomiser_csv[ (exomiser_csv['sift'].isnull()) | (exomiser_csv['sift'] < 0.3) ]

# remove low polyphen
exomiser_csv['poly_phen'] = pd.to_numeric(exomiser_csv['poly_phen'], errors='coerce')
exomiser_csv  = exomiser_csv[(exomiser_csv['poly_phen'].isnull()) | (exomiser_csv['poly_phen'] > 0.115) ]
